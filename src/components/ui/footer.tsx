...
const Footer = () => {
  ...
  const getInvolvedLinks = [
    { href: '/contact', label: 'Join a Program' },
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <a
                    ...
                    className={`w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center text-white transition-all duration-300 ${social.hoverColor} hover:scale-110`}
            ...
            <nav className="space-y-3">
              {quickLinks.map((link) => (
                <Link
                  key={link.href}
                  to={link.href}
                  className="block text-footer-text/80 hover:text-white transition-colors duration-200 hover:translate-x-1 transform"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link to="/about">
                <Button 
                  variant="outline" 
                  size="lg"
                  className="border-white/20 text-white hover:bg-white/10 font-body font-semibold"
                >
                  Learn More
                </Button>
              </Link>
